...
test_that("bfa.t.test.default accepts the same format as t.test.default", {
  ...
  bayes.t.test(1:10, y = c(7:20))      # P = .00001855
  bayes.t.test(1:10, y = c(7:20, 200)) # P = .1245    -- NOT significant anymore
...
test_that("bfa_t_test returns bfa_t_test* object of the right class", {
  ...
  test_that(bayes.t.test(x, y), is_a("bfa_two_sample_t_test"))
  test_that(bayes.t.test(x), is_a("bfa_one_sample_t_test"))
  test_that(bayes.t.test(x, y, paired=TRUE), is_a("bfa_paired_t_test"))
})
