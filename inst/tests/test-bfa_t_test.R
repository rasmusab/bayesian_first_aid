...
test_that("bfa.t.test.default accepts the same format as t.test.default", {
  ...
  bfa.t.test(1:10, y = c(7:20))      # P = .00001855
  bfa.t.test(1:10, y = c(7:20, 200)) # P = .1245    -- NOT significant anymore
  with(sleep, bfa.t.test(extra[group == 1], extra[group == 2]))
})
...
test_that("bfa_t_test returns bfa_t_test* object of the right class", {
  ...
  test_that(bfa.t.test(x, y), is_a("bfa_two_sample_t_test"))
  test_that(bfa.t.test(x), is_a("bfa_one_sample_t_test"))
  test_that(bfa.t.test(x, y, paired=TRUE), is_a("bfa_paired_t_test"))
})
