...
test_that("bfa.binom.test accepts the same format as binom.test and returns a bfa_binom_test object", {
  ...
  expect_that(bayes.binom.test(c(682, 243), p = 3/4), is_a("bfa_binom_test"))
  expect_that(bayes.binom.test(682, 682 + 243, p = 3/4), is_a("bfa_binom_test"))
})
...
test_that("bfa.binom.test with generic function does not give an error when run", {
  fit <- bayes.binom.test(6, 11, p=0.84, alternative="two.sided")
